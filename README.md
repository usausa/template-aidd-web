...
```
- **各段の具体プロンプト(コピペ可)とコマンド早見表**は `docs/guides/workflow.md`(正はそちら。ここに複製しない)。
...
| `commands/*.md` | **人** `/...` | 開発ループの各段を起動(一覧は `commands/` を見る) |
| `agents/*.md` | 委譲 | サブエージェント定義(一覧は `agents/` を見る) |
| `skills/*/SKILL.md` | 自動ロード | 作業手順(実装・ADR・生成・蒸留 等。一覧は `skills/` を見る) |
<!-- pm:guide-claude -->
...
- **正の一覧(寿命クラス表)と退役・ID のルールは `docs/README.md`、AI 常時ルールは `AGENTS.md`**(この README は入口なので置き換え可)。
